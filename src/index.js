export default function() {
    let nodes,
        ...
        massAccessor = (node => Math.pow(radiusAccessor(node), 2)),  // Mass proportional to area by default
        onImpact;
    ...
    function force() {
        nodes.forEach(a => {
            nodes.filter(b => a !== b).forEach(b => {
                ...
                ({ a: vaRel.x, b: vbRel.x } = getAfterImpactVelocities(massAccessor(a), massAccessor(b), vaRel.x, vbRel.x, elasticity));
    ...
    force.massAccessor = function(_) {
        return arguments.length ? (massAccessor = _, force) : massAccessor;
    };
